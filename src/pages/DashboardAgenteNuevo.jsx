// ====================================
// DASHBOARD DEL AGENTE IA - VERSIÓN NUEVA CON NO-SHOWS
// 🚨 PÁGINA DE PRUEBA - NO REEMPLAZA LA ORIGINAL
// ====================================
...
import { 
    ...
    Shield, Brain, Activity, Clock, CheckCircle, Phone, Link as LinkIcon
} from 'lucide-react';
...
// Importar componentes nuevos de No-Shows
import NoShowAlertCard from '../components/noshows/NoShowAlertCard';
import { useChannelStats } from '../hooks/useChannelStats';

export default function DashboardAgenteNuevo() {
    ...
    // 🆕 Estados nuevos para No-Shows y Canales
    const [activeAlerts, setActiveAlerts] = useState([]);
    const [channelsData, setChannelsData] = useState({
        vapi: { active: false, count: 0 },
        whatsapp: { active: false, count: 0 },
        instagram: { active: false, count: 0 },
        facebook: { active: false, count: 0 },
        webchat: { active: false, count: 0 }
    });
    const { channelStats, loading: channelsLoading } = useChannelStats();
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            // 🆕 11. CARGAR ALARMAS ACTIVAS DE NO-SHOWS (T-2h 15min)
            ...
            // 🆕 12. CARGAR RESERVAS POR CANAL HOY
            ...
            // Contar por canal
            const channelCounts = (channelReservations || []).reduce((acc, r) => {
            ...
            setChannelsData({
                vapi: { active: channelStats?.validChannels?.includes('vapi') || false, count: channelCounts['vapi'] || 0 },
                whatsapp: { active: channelStats?.validChannels?.includes('whatsapp') || false, count: channelCounts['whatsapp'] || 0 },
                instagram: { active: channelStats?.validChannels?.includes('instagram') || false, count: channelCounts['instagram'] || 0 },
                facebook: { active: channelStats?.validChannels?.includes('facebook') || false, count: channelCounts['facebook'] || 0 },
                webchat: { active: channelStats?.validChannels?.includes('web_chat') || false, count: channelCounts['web'] || 0 }
            });
    ...
    // 🆕 Función para resolver alarma (después de llamar)
        ...
        <div className="min-h-screen bg-gray-50 p-4 md:p-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                    ...
                    {/* 🆕 CANALES ACTIVOS (reemplaza No-Shows) */}
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col hover:shadow-md transition-shadow">
                        <div className="p-6 flex-1">
                            <div className="flex items-start justify-between mb-4">
                                <div>
                                    <p className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-2">Canales Activos</p>
                                    <h3 className="text-4xl font-bold text-gray-900">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex items-center justify-between text-xs">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${channelsData.vapi.active ? 'bg-green-500' : 'bg-gray-300'}`} />
                                        <span className="text-gray-700">VAPI (Llamadas)</span>
                                    </div>
                                    <span className="font-semibold text-gray-900">{channelsData.vapi.count}</span>
                                </div>
                                {/* WhatsApp */}
                                <div className="flex items-center justify-between text-xs">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${channelsData.whatsapp.active ? 'bg-green-500' : 'bg-gray-300'}`} />
                                        <span className="text-gray-700">WhatsApp</span>
                                    </div>
                                    <span className="font-semibold text-gray-900">{channelsData.whatsapp.count}</span>
                                </div>
                                {/* Instagram */}
                                <div className="flex items-center justify-between text-xs">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${channelsData.instagram.active ? 'bg-green-500' : 'bg-gray-300'}`} />
                                        <span className="text-gray-700">Instagram</span>
                                    </div>
                                    <span className="font-semibold text-gray-900">{channelsData.instagram.count}</span>
                                </div>
                                {/* Facebook */}
                                <div className="flex items-center justify-between text-xs">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${channelsData.facebook.active ? 'bg-green-500' : 'bg-gray-300'}`} />
                                        <span className="text-gray-700">Facebook</span>
                                    </div>
                                    <span className="font-semibold text-gray-900">{channelsData.facebook.count}</span>
                                </div>
                                {/* Web Chat */}
                                <div className="flex items-center justify-between text-xs">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${channelsData.webchat.active ? 'bg-green-500' : 'bg-gray-300'}`} />
                                        <span className="text-gray-700">Web Chat</span>
                                    </div>
                                    <span className="font-semibold text-gray-900">{channelsData.webchat.count}</span>
                                </div>
                            </div>
                        </div>
