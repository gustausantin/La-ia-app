// Dashboard.jsx - Versión ACTUALIZADA con nomenclatura unificada
...
// - tabla: conversations (antes message_batches_demo - conversaciones del agente)
// - tabla: agent_metrics (métricas diarias del agente)
// - RPC: get_agent_daily_stats(restaurant_id, date)
// - RPC: get_reservation_stats_by_source(restaurant_id, date)
// - real-time: suscripción a reservations y conversations
...
function Dashboard() {
    ...
    const { 
        ...
        restaurantId, 
        isReady, 
        addNotification,  // NUEVO - para notificaciones globales
        agentStatus 
    } = useAuthContext();
    ...
    const fetchDashboardStats = useCallback(async () => {
        ...
        try {
            ...
            //     { p_restaurant_id: restaurantId, p_date: new Date() }
            // );
    ...
    // Función para obtener conversaciones del agente - ACTUALIZADO
    const fetchAgentConversations = useCallback(async () => {
        ...
        try {
            ...
            const { data, error } = await supabase
                .from('conversations')  // ACTUALIZADO de 'message_batches_demo'
                .select(`
                    id,
                    customer_id,
                    customer_name,
    ...
    // Función para obtener reservas del día - ACTUALIZADO
    const fetchTodayReservations = useCallback(async () => {
        ...
        try {
            ...
            const reservationsWithSource = (data || []).map(res => {
                ...
                // TODO: Eliminar cuando la BD tenga estos campos
                return {
                    ...res,
        ...
        const channel = supabase
            ...
            .on(
                "postgres_changes",
                {
                    event: "INSERT",
                    schema: "public",
                    table: "conversations",  // ACTUALIZADO de 'message_batches_demo'
                    filter: `restaurant_id=eq.${restaurantId}`,
                },
                (payload) => {
                    console.log("Nueva conversación:", payload);
                    setConversations((prev) => [payload.new, ...prev].slice(0, 10));
                    ...
                    addNotification({
                        type: 'agent',
                        message: `Nueva conversación en ${payload.new.channel}`,
                        priority: 'normal'
                    });
                },
            )
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            {!agentStatus.active && (
                <Alert
                    ...
                    message="El agente no está procesando reservas automáticamente. Actívalo para aprovechar todo el potencial de Son-IA."
                    actionText="Activar Agente"
                    onAction={() => {
                        // Navegar a configuración o activar directamente
