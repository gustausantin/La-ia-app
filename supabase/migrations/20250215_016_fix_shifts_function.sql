-- CORRECCIÓN: Eliminar función existente antes de recrear con sistema de turnos
-- Esto evita el error 409 de conflicto

-- 1. Eliminar función existente
DROP FUNCTION IF EXISTS generate_availability_slots(uuid,date,date);

-- 2. Recrear con sistema de turnos
    ...
    WHILE current_loop_date <= final_end_date LOOP
        ...
        IF day_schedule IS NOT NULL AND COALESCE((day_schedule->>'closed')::BOOLEAN, false) = false THEN
            ...
            IF NOT is_day_affected THEN
                ...
                -- NUEVA LÓGICA: Usar turnos (shifts) si existen
                ...
                IF day_shifts IS NOT NULL AND jsonb_array_length(day_shifts) > 0 THEN
                    ...
                    FOR i IN 0..jsonb_array_length(day_shifts) - 1 LOOP
                        ...
                        BEGIN
                            ...
                            -- Generar slots solo en este turno
                ...
                ELSE
                    -- FALLBACK: Usar horario completo si no hay turnos
                    DECLARE
