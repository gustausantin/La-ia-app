-- MIGRACIÓN: Función compatible con llamadas con y sin buffer
-- Fecha: 18 Septiembre 2025
-- Objetivo: Crear función que ignore el buffer pero sea compatible
...
-- Crear función que ignora el buffer (pero lo acepta para compatibilidad)
...
BEGIN
    ...
    RAISE NOTICE 'FINAL LOGIC: Buffer ignored (was %), only duration matters. Table: is_available=%, status=%', p_buffer_minutes, has_is_available, has_status;
    ...
    RAISE NOTICE 'Generating slots from % to % (BUFFER IGNORED)', p_start_date, final_end_date;
    ...
    RAISE NOTICE 'Successfully created % slots (BUFFER WAS IGNORED)', slots_created;
