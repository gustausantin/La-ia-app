-- ACTUALIZACIÓN: Generar disponibilidades solo en turnos configurados
-- No en todo el horario de apertura del restaurante
...
BEGIN
    ...
    WHILE current_loop_date <= final_end_date LOOP
        ...
        -- Verificar si el día está configurado y abierto
        IF day_schedule IS NOT NULL AND COALESCE((day_schedule->>'closed')::BOOLEAN, false) = false THEN
            ...
            -- Si el día no está afectado por cierres, procesar turnos
            IF NOT is_day_affected THEN
                
                -- NUEVA LÓGICA: Usar turnos (shifts) en lugar del horario completo
                ...
                IF day_shifts IS NOT NULL AND jsonb_array_length(day_shifts) > 0 THEN
                    ...
                    FOR i IN 0..jsonb_array_length(day_shifts) - 1 LOOP
                        ...
                        DECLARE
                            shift_data JSONB := day_shifts->i;
                            start_time_str TEXT := shift_data->>'start_time';
                            end_time_str TEXT := shift_data->>'end_time';
                        BEGIN
                            -- Parsing robusto de horarios del turno
                            BEGIN
                                ...
                                ELSE
                                    CONTINUE; -- Saltar este turno si no tiene hora válida
                                END IF;
                            EXCEPTION WHEN OTHERS THEN
                                CONTINUE; -- Saltar este turno si hay error de parsing
                            END;
                            ...
                            -- Validar que el turno es lógico
                                ...
                                LOOP
                                    ...
                                    WHILE current_slot_time + ((turn_duration + buffer_minutes) || ' minutes')::INTERVAL <= shift_end LOOP
                                        ...
                                        WHERE r.restaurant_id = p_restaurant_id
                                          ...
                                          AND r.status IN ('confirmed', 'seated') -- Usar nombres en inglés de BD
                                          AND (
                                        ...
                                        IF existing_reservations = 0 THEN
                                            INSERT INTO availability_slots (
                                                ...
                                                'free',
                                                'system',
                                                jsonb_build_object('shift_name', shift_data->>'name', 'shift_id', shift_data->>'id'),
                                                NOW(),
                                                NOW()
                                            );
                ...
                ELSE
                    -- FALLBACK: Si no hay turnos configurados, usar horario completo (comportamiento anterior)
                    DECLARE
                        open_value TEXT := day_schedule->>'open';
                        close_value TEXT := day_schedule->>'close';
                        closed_value BOOLEAN := COALESCE((day_schedule->>'closed')::BOOLEAN, false);
                    BEGIN
                        IF NOT closed_value THEN
                            ...
                            -- Generar slots en horario completo (fallback)
                            IF shift_start < shift_end THEN
                                FOR table_record IN 
                                    SELECT id, name, capacity, is_active 
                                    FROM tables 
                                    ...
                                    WHILE current_slot_time + ((turn_duration + buffer_minutes) || ' minutes')::INTERVAL <= shift_end LOOP
                                        ...
                                        IF existing_reservations = 0 THEN
                                            INSERT INTO availability_slots (
                                                ...
                                                'free',
                                                'system',
                                                jsonb_build_object('fallback_mode', true),
                                                NOW(),
                                                NOW()
                                            );
...
'Función ultra-robusta para generar slots de disponibilidad SOLO en turnos configurados. Si no hay turnos, usa horario completo como fallback.';
