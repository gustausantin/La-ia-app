-- MIGRACIÓN: Corregir error de clave duplicada
-- Fecha: 18 Septiembre 2025
-- Objetivo: Manejar duplicados con ON CONFLICT DO NOTHING

DROP FUNCTION IF EXISTS generate_availability_slots(uuid, date, date, integer, integer) CASCADE;

-- Función con manejo de duplicados
...
BEGIN
    ...
    RAISE NOTICE 'DUPLICATE FIX: ON CONFLICT handling. Buffer ignored (was %), only duration matters. Table: is_available=%, status=%', p_buffer_minutes, has_is_available, has_status;
    ...
    -- LIMPIEZA AGRESIVA: Eliminar TODOS los slots existentes en el rango
    ...
    RAISE NOTICE 'Successfully created % slots (DUPLICATES HANDLED)', slots_created;
