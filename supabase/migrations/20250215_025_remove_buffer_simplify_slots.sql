-- MIGRACIÓN: Eliminar buffer y simplificar lógica de slots
-- Fecha: 18 Septiembre 2025
-- Objetivo: SIN BUFFER - Solo duración de reserva. Turno 12:00-14:00 + 60min = slots: 12:00, 13:00, 14:00

DROP FUNCTION IF EXISTS generate_availability_slots(uuid, date, date, integer, integer) CASCADE;

-- Función simplificada SIN BUFFER
...
BEGIN
    ...
    RAISE NOTICE 'SIMPLE LOGIC: No buffer, only slot duration. Table structure: is_available=%, status=%', has_is_available, has_status;
    ...
    RAISE NOTICE 'Generating slots from % to % (NO BUFFER - SIMPLE LOGIC)', p_start_date, final_end_date;
    ...
    WHILE loop_date <= final_end_date LOOP
        ...
        IF day_shifts IS NOT NULL AND jsonb_array_length(day_shifts) > 0 THEN
            ...
            LOOP
                ...
                LOOP
                    ...
                    -- LÓGICA SIMPLE: Solo mientras el slot + duración quepan dentro del turno
                    -- Ejemplo: Turno 12:00-14:00, duración 60min → slots: 12:00, 13:00, 14:00 (el último termina a las 15:00 pero empieza dentro del turno)
                    WHILE slot_time + (p_slot_duration_minutes || ' minutes')::interval <= shift_end_time + (p_slot_duration_minutes || ' minutes')::interval LOOP
                        end_time := slot_time + (p_slot_duration_minutes || ' minutes')::interval;
                        
                        RAISE NOTICE 'Creating slot for table % at % (shift: %, ends at: %)', table_record.name, slot_time, shift_record->>'name', end_time;
                        ...
                        -- Avanzar al siguiente slot (SOLO duración, SIN buffer)
        ...
        ELSE
            ...
            LOOP
                ...
                -- LÓGICA SIMPLE: Solo mientras el slot + duración quepan dentro del horario
                WHILE slot_time + (p_slot_duration_minutes || ' minutes')::interval <= shift_end_time + (p_slot_duration_minutes || ' minutes')::interval LOOP
                    end_time := slot_time + (p_slot_duration_minutes || ' minutes')::interval;
    ...
    RAISE NOTICE 'Successfully created % slots (NO BUFFER LOGIC)', slots_created;
