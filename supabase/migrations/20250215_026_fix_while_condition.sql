-- MIGRACIÓN: Corregir condición WHILE para generar slots correctos
-- Fecha: 18 Septiembre 2025
-- Objetivo: WHILE slot_time <= shift_end_time (no slot_time + duration)

DROP FUNCTION IF EXISTS generate_availability_slots(uuid, date, date, integer) CASCADE;

-- Función con WHILE corregido
...
BEGIN
    ...
    RAISE NOTICE 'FIXED LOGIC: WHILE slot_time <= shift_end_time. Table structure: is_available=%, status=%', has_is_available, has_status;
    ...
    RAISE NOTICE 'Generating slots from % to % (FIXED WHILE CONDITION)', p_start_date, final_end_date;
        ...
        IF day_shifts IS NOT NULL AND jsonb_array_length(day_shifts) > 0 THEN
            ...
            LOOP
                ...
                LOOP
                    ...
                    -- LÓGICA CORREGIDA: Solo mientras slot_time <= shift_end_time
                    -- Ejemplo: Turno 12:00-14:00 → slots: 12:00, 13:00, 14:00
    ...
    RAISE NOTICE 'Successfully created % slots (FIXED WHILE CONDITION)', slots_created;
