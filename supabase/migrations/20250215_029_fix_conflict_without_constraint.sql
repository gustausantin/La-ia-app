-- MIGRACIÓN: Corregir ON CONFLICT sin especificar constraint
-- Fecha: 18 Septiembre 2025
-- Objetivo: Usar IF NOT EXISTS en lugar de ON CONFLICT

DROP FUNCTION IF EXISTS generate_availability_slots(uuid, date, date, integer, integer) CASCADE;

-- Función con IF NOT EXISTS para evitar duplicados
CREATE OR REPLACE FUNCTION generate_availability_slots(
    p_restaurant_id uuid,
    p_start_date date,
    p_end_date date,
    p_slot_duration_minutes integer DEFAULT 90,
    p_buffer_minutes integer DEFAULT 0  -- IGNORADO - Solo para compatibilidad
)
RETURNS integer
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    RAISE NOTICE 'IF NOT EXISTS approach: Buffer ignored (was %), only duration matters. Table: is_available=%, status=%', p_buffer_minutes, has_is_available, has_status;
    ...
    -- LIMPIEZA COMPLETA: Eliminar TODOS los slots existentes en el rango
    DELETE FROM availability_slots 
    WHERE restaurant_id = p_restaurant_id 
    AND slot_date BETWEEN p_start_date AND final_end_date;
    
    RAISE NOTICE 'Cleaned ALL existing slots from % to %', p_start_date, final_end_date;
    ...
    WHILE loop_date <= final_end_date LOOP
        ...
        IF day_schedule IS NULL OR (day_schedule->>'open')::boolean = false THEN
            ...
            LOOP
                -- Verificar si ya existe el slot de día cerrado
                IF NOT EXISTS (
                    SELECT 1 FROM availability_slots 
                    WHERE restaurant_id = p_restaurant_id 
                    AND table_id = table_record.id 
                    AND slot_date = loop_date 
        ...
        IF day_shifts IS NOT NULL AND jsonb_array_length(day_shifts) > 0 THEN
            ...
            LOOP
                ...
                LOOP
                    ...
                    -- LÓGICA FINAL: Solo mientras slot_time <= shift_end_time
                    -- Turno 12:00-14:00, duración 60min → slots: 12:00, 13:00, 14:00
                    WHILE slot_time <= shift_end_time LOOP
                        end_time := slot_time + (p_slot_duration_minutes || ' minutes')::interval;
                        
                        RAISE NOTICE 'Creating slot: table=%, time=%, shift=%, ends=%', table_record.name, slot_time, shift_record->>'name', end_time;
    ...
    RAISE NOTICE 'Successfully created % slots (IF NOT EXISTS approach)', slots_created;
