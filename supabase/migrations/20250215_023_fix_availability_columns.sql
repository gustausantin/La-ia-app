-- MIGRACIÓN: Corregir función para usar columnas correctas de availability_slots
-- Fecha: 18 Septiembre 2025
-- Objetivo: Adaptar la función a la estructura real de la tabla

-- Primero verificar si la columna is_available existe, si no, crearla
DO $$
BEGIN
    -- Intentar agregar la columna is_available si no existe
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'availability_slots' 
        AND column_name = 'is_available'
    ) THEN
        ALTER TABLE availability_slots ADD COLUMN is_available boolean DEFAULT true;
        RAISE NOTICE 'Added is_available column to availability_slots';
    END IF;
    
    -- Verificar si existe la columna status en su lugar
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'availability_slots' 
        AND column_name = 'status'
    ) THEN
        RAISE NOTICE 'Table has status column instead of is_available';
    END IF;
END $$;
